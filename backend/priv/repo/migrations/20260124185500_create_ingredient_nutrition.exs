defmodule Controlcopypasta.Repo.Migrations.CreateIngredientNutrition do
  ...
  def change do
    ...
    create table(:ingredient_nutrition, primary_key: false) do
      ...
      add :source_id, :string  # External ID (e.g., USDA FDC ID: "171287")
      add :source_name, :string  # Human-readable source description
      add :source_url, :string  # Link to source data

      # Reference amount (what the nutrition values are based on)
      add :serving_size_value, :decimal, null: false  # e.g., 100
      add :serving_size_unit, :string, null: false    # e.g., "g", "ml"
      add :serving_description, :string               # e.g., "1 cup chopped", "1 medium"

      # Macronutrients (all per serving_size)
      add :calories, :decimal           # kcal
      add :protein_g, :decimal          # grams
      add :fat_total_g, :decimal        # grams
      add :fat_saturated_g, :decimal    # grams
      add :fat_trans_g, :decimal        # grams
      add :fat_polyunsaturated_g, :decimal
      add :fat_monounsaturated_g, :decimal
      add :carbohydrates_g, :decimal    # grams
      add :fiber_g, :decimal            # grams
      add :sugar_g, :decimal            # grams
      add :sugar_added_g, :decimal      # grams
      ...
      # Vitamins
      add :vitamin_a_mcg, :decimal      # RAE (retinol activity equivalents)
      ...
      add :thiamin_mg, :decimal         # B1
      add :riboflavin_mg, :decimal      # B2
      add :niacin_mg, :decimal          # B3
      ...
      # Metadata
      add :is_primary, :boolean, default: false  # Preferred source for this ingredient
      add :verified_at, :utc_datetime           # When manually verified
      add :notes, :text                         # Any notes about the data
